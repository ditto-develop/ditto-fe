...
애플은 Sign in with Apple 을 제공하면서 계정 삭제도 제공하는 앱에
`POST https://appleid.apple.com/auth/revoke` 로 **토큰 폐기를 의무화**한다. 가이드라인
5.1.1(v)와 묶여 있어 안 하면 리젝 사유다. 탈퇴 화면(`/settings/withdraw`)은 이미 완성돼
있으므로, 애플 로그인을 켜는 순간 이 요구가 그대로 걸린다.

**FE 가 할 수 없는 일이다.** 폐기 요청에는 팀 비공개 키(`.p8`)로 ES256 서명한
client_secret JWT 가 필요하다. 이 앱은 원격 URL 로드라 웹 번들이 곧 공개 자산이므로,
그 키가 프런트에 있으면 **누구나 아무 사용자의 애플 토큰을 폐기할 수 있다.**

**BE 가 해야 하는 것**

1. 최초 로그인에서 `authorizationCode` 를 받아 `POST appleid.apple.com/auth/token` 으로
   교환하고 **refresh token 을 저장**한다. (지금은 인가 코드 교환을 하지 않는다 — 위키 §2)
2. 탈퇴 처리에서 그 refresh token 으로 `/auth/revoke` 를 호출한다.
3. client_secret 용 **Sign in with Apple 전용 `.p8`** 발급(푸시 APNs 키와 다른 키).
   수명 최대 6개월이라 갱신 경로가 필요하다.

**BE 가 1번을 받기로 하면 FE 변경은 한 줄이다.** 네이티브 플러그인이 `authorizationCode`
를 이미 돌려주고 있으므로 `loginWithExternalAppleNative` 의 body 에 한 필드를 더하면 된다.
...
### B-6. 가입 `age` — 하한값 vs 중앙값, **BE 회신 필요**
...
FE 는 **하한값**을 보낸다 — `toAgeBucket`(`src/shared/lib/age.ts`)이
`[60, 50, 45, 40, 35, 30, 25, 20]` 중 하한을 고른다(`toAgeBucket(27) === 25`).

한 바퀴 돌아 어긋난 것이다. FE 는 원래 중앙값을 보냈고, 서버 문서의 "20, 25, 30…"을
보고 하한값으로 바꿨다(`Tutorial.tsx` 주석에 그 경위가 남아 있었다). 그런데 BE 는 그 문서가
오기였다고 정정했다.

**영향은 처음 적은 것보다 작다.**

- 표시는 무관하다. `formatAgeRange`(`src/shared/lib/formatAge.ts`)는 값을 **구간으로 분류**하므로
  22 든 25 든 "20~24세"/"25~29세"로 옳게 나온다. Tutorial 주석에 있던
  "`formatAgeRange` 가 하한값을 전제한다"는 **사실이 아니었고**, 이번에 고쳤다.
- 매칭도 대부분 상쇄된다. BE 필터가 나이차 10 이내이고 두 사람 모두 같은 방식으로
  버킷되므로, 하한이든 중앙이든 **차이값은 거의 같다.** 어긋나는 곳은 폭이 다른 구간
  (5년 구간들 vs 10년짜리 `50~59`) 경계뿐이다.

**그래도 추정해서 바꾸지 않는다.** 위키의 예시가 셋뿐인데(22 / 27 / 60) FE 구간에는 45·50 이
있고 55 가 없어 `45~49`·`50~59` 의 중앙값이 정해지지 않는다. 애초에 구간 격자가 5년 단위가
맞는지도 확인이 필요하다. → **BE 에 전체 매핑표를 받고 나서 `toAgeBucket` 한 곳만 고친다.**

### B-7. 가입 필수값 가드 — ✅ 적용 완료 (2026-09-07)

같은 위키 §2 로 `gender`·`age` 가 가입 필수가 됐다(누락 시 `0001`). FE 는 값이 없으면
`age: 0` · `gender: ""` 로 요청을 보내고 있었다.

정상 흐름에서는 Step2 검증이 둘 다 보장하지만 **그 검증을 건너뛰는 경로가 둘 있었다** —
`handleNext` 의 `if (step2Ref.current && ...)` 는 참조가 비면 통째로 건너뛰고,
`handleSkip`(소개노트 건너뛰기)은 검증 없이 곧장 가입을 쏜다. 그 경우 서버가 거절하고
사용자에게는 "회원가입 중 문제가 발생했어요"라는 원인 없는 문구만 보인다.

→ 요청 직전 한 곳(`Tutorial.tsx` goNextStep)에서 막고 무엇을 채워야 하는지 알린다.
