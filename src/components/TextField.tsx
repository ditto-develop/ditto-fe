...
export const TextField: React.FC<TextFieldProps> = ({
  ...
  return (
    <FieldRoot>
      ...
      {isessential && <Label1Normal $color="var(--color-status-destructive)">*</Label1Normal>}
      </div>}
...
const RightArea = styled.div<{ $isborder?: boolean }>`
  ...
  $isborder ? "1px solid var(--color-line-neutral)" : "none"};
`;
...
const FieldLabel = styled(Label1Normal)`
  color: var(--Color-Label-Normal, #1a1815);
`;
...
const HelperText = styled(Caption1)<{ $status: InputStatus }>`
  color: ${({ $status }) => {
    switch ($status) {
      case "success":
        return "var(--color-line-normal, #557a55)";
      case "error":
        return "var(--color-Status-Negative, #e03131)";
      case "disabled":
        return "rgba(47, 43, 39, 0.35)";
      default:
        return "var(--color-Label-Alternative, rgba(47, 43, 39, 0.61))";
    }
  }};
`;
