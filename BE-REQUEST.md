...
|---|---|---|---|---|---|
| 1 | 알림 센터 3종 | 개발 | **P0** | 화면 완성, MSW 목업으로만 동작 | `A-1` |
...
`/notifications` 화면이 **MSW 목업으로만 동작합니다.** 배포 워크플로우에 `NEXT_PUBLIC_API_MOCKING`이 없어 목업이 꺼져 있으므로, staging/prod에서 이 화면은 그냥 비어 있습니다.

BE 위키 MyPage 가이드에도 *"알림 센터(받은 알림 목록·읽음) — 미구현. 설정의 알림 토글과 별개 기능"* 으로 명시돼 있습니다. 알림 **설정** 토글(`GET`/`PATCH /api/v1/users/me/notification-settings`)은 이미 라이브인데, 정작 알림 자체가 없는 상태입니다.

화면(Figma `7.2 알림 센터`)은 이미 완성돼 있습니다 — 필터 4탭, 오늘/지난 소식 구간, 상대 시간 5단계, 개별·전체 읽음, 빈 상태. **API만 뜨면 목업 핸들러만 걷어냅니다.**
...
`GET` 응답 항목 스키마 (FE `NotificationItem`):

```jsonc
{
  "id": "string",
  "type": "MATCH_RESULT | NEW_MESSAGE | GROUP_FORMED | VOTE_RESULT | RATING_REQUEST | REMATCH_SUCCESS | CHAT_CLOSING | APP_UPDATE",
  "title": "이번 주 매칭 결과가 나왔어요",
  "body": "나와 답변이 비슷한 사람들을 찾았어요. 지금 확인해 보세요.",
  "createdAt": "2026-08-18T09:12:00+09:00",   // ISO 8601
  "read": false,
  "linkTo": "/matching"                        // 탭 시 이동할 앱 내 경로. 없으면 null
}
```

- `type`은 목록 아이콘과 필터 탭(전체/매칭/대화/시스템) 분류에 쓰입니다. **위 8종이 FE가 아는 전부**이며, 모르는 값이 오면 FE가 렌더하지 않습니다. 종류를 늘리실 거면 알려주세요.
- `linkTo`는 서버가 앱 내 경로를 직접 내려주는 형태를 가정했습니다. 대신 `targetType` + `targetId`로 내려주시면 FE가 경로를 조립해도 됩니다 — 편하신 쪽으로.
- 정렬은 FE가 `createdAt` 내림차순으로 다시 하므로 순서는 상관없습니다.
- 읽음 처리는 **`POST`로 제안**합니다(상태 전이라 `PUT`보다 맞다고 봤습니다). BE가 `PUT`을 선호하시면 FE는 한 줄이니 그대로 따르겠습니다.

### 함께 확인 (→ 9번)

Figma 스펙 표가 알림 목록을 "최근 30일"로 적어 두었습니다. **서버가 30일로 잘라 줄지, 전건을 주고 FE가 자를지** 정해 주세요. 현재 FE는 받은 전건을 그립니다.
...
1번에 포함된 항목입니다. Figma 스펙 표가 알림 목록을 "최근 30일"로 정의했는데, **서버가 30일로 잘라 줄지 / 전건을 주고 FE가 자를지** 정해 주세요. 현재 FE는 받은 전건을 오늘·지난 소식 2구간으로 나눠 그립니다.
...
- [ ] **(9)** 알림 목록 30일 컷은 서버/FE 중 어디서?
- [ ] **(1)** 읽음 처리 메서드 `POST` vs `PUT`
- [ ] **(1)** `linkTo`(완성 경로) vs `targetType`+`targetId`(FE 조립) 중 어느 쪽?
